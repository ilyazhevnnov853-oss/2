    ...
    <div className="flex items-center justify-between p-4 mb-6 liquid-glass rounded-3xl shrink-0 relative z-30 mx-4 mt-4">
        <div className="flex items-center gap-4">
            <div className="flex gap-2">
                ...
                {onBack && (
                    <button onClick={onBack} className="p-2.5 rounded-2xl bg-white/5 hover:bg-white/10 text-slate-400 hover:text-white transition-all active:scale-95 border border-white/5">
                        <ChevronLeft size={18} />
                    </button>
                )}
            </div>
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    ...
                    {subtitle && <p className="text-[10px] text-blue-200/60 font-bold uppercase tracking-widest mt-1">{subtitle}</p>}
                </div>
            </div>
        </div>
...
export const GlassMetric = ({ label, value, unit, color }: any) => (
    <div className="flex flex-col items-center min-w-[90px] p-2 rounded-2xl hover:bg-white/5 transition-colors cursor-default">
        <span className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1">{label}</span>
        <div className="flex items-baseline gap-1">
            <span className={`font-mono text-xl font-black ${color} drop-shadow-md`}>{value}</span>
    ...
    return (
        <div className="group select-none">
            ...
            <div className="relative h-5 w-full touch-none flex items-center cursor-pointer">
                ...
                {/* Handle (Glowing Orb) */}
                <div 
                    className="absolute h-6 w-6 bg-white rounded-full shadow-[0_0_15px_rgba(255,255,255,0.5)] border-4 border-[#0f172a] z-10 pointer-events-none transition-all duration-75 group-hover:scale-110 flex items-center justify-center" 
