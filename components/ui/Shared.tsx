    ...
    <div className="flex items-center justify-between p-4 mb-6 bg-white/60 dark:bg-[#14141e]/40 backdrop-blur-2xl rounded-3xl shrink-0 relative z-30 mx-4 mt-4 shadow-sm dark:shadow-none border border-black/5 dark:border-white/5 transition-colors duration-500">
        <div className="flex items-center gap-4">
            <div className="flex gap-2">
                <button onClick={onHome} aria-label="Главная" className="p-2.5 rounded-2xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-slate-500 dark:text-slate-400 hover:text-black dark:hover:text-white transition-all active:scale-95 border border-transparent dark:border-white/5 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none">
                    <Home size={18} />
                </button>
                {onBack && (
                    <button onClick={onBack} aria-label="Назад" className="p-2.5 rounded-2xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-slate-500 dark:text-slate-400 hover:text-black dark:hover:text-white transition-all active:scale-95 border border-transparent dark:border-white/5 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none">
                        <ChevronLeft size={18} />
                    </button>
                )}
            </div>
...
export const GlassButton = ({ onClick, icon, label, active, secondary, customClass, disabled, ariaLabel }: any) => {
    let base = "relative overflow-hidden group h-12 rounded-2xl flex items-center justify-center gap-2 transition-all font-bold text-xs uppercase tracking-wide shadow-md active:scale-95 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none ";
    ...
    return (
        <button onClick={onClick} disabled={disabled} aria-label={ariaLabel || label} className={`${base} ${style}`}>
        ...
        <div className="group select-none">
            ...
            <div className="relative h-5 w-full touch-none flex items-center cursor-pointer">
                ...
                <input 
                    ...
                    aria-label={label}
                    className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-20 focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:outline-none rounded-full"
                />
