...
const DiffuserCanvas: React.FC<DiffuserCanvasProps> = ({ 
  ...
  onDragEnd
    ...
    const renderTopView = (ctx: CanvasRenderingContext2D) => {
        ...
        // Draw Floor with Highlight if Dragging
        if (dragPreview) {
            ctx.fillStyle = '#1e293b'; // Lighter blue-slate when dragging
        ...
        if (dragPreview) {
            ...
            // Physical Size from Props
            const wPx = dragPreview.width * ppm;
            const hPx = dragPreview.height * ppm;
            
            // 1. Coverage Area Ghost (Approximate)
            ctx.beginPath();
            // A roughly 2m radius ghost for context (or from spec if we calculated it, but let's keep it simple)
            ctx.arc(cx, cy, 2.0 * ppm, 0, Math.PI * 2);
            ctx.fillStyle = 'rgba(255, 255, 255, 0.05)';
            ctx.fill();
            
            // 2. Diffuser Body Ghost
            ctx.beginPath();
            ctx.rect(cx - wPx/2, cy - hPx/2, wPx, hPx);
            ctx.fillStyle = 'rgba(59, 130, 246, 0.5)'; // Stronger blue
            ctx.fill();
            ...
            ctx.textAlign = 'center';
            ctx.fillText(`X:${dragPreview.x.toFixed(1)} Y:${dragPreview.y.toFixed(1)}`, cx, cy - hPx/2 - 8);
